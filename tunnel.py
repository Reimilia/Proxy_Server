...
class SenderFactory(protocol.ClientFactory):
    ...
    def buildProtocol(self, *a, **kw) :
        return protocol.ClientFactory.buildProtocol(self, *a, **kw).setReceiver(self._receiver)
...
class Options(ServerOptions):
    ...
    def parseOptions(self, *a, **kw):
        ...
        if self.get("base")[0] in ("https", ):
            if not self.get("ssl-priv-file") or not self.get("ssl-cert-file"):
                raise usage.UsageError("For `https`, `ssl-priv-file` and `ssl-cert-file` must be given.")

    def opt_base(self, value):
        try:
            (_proto, _host_and_port, ) = value.split(":", 1, )
        except ValueError:
            raise usage.UsageError("invalid format of `base`, '<protocol>:<host address>[:<port>]'")
        else :
            if _proto.lower() not in SUPPORTED_PROTOCOLS:
                raise usage.UsageError("invalid `protocol`, %s" % ", ".join(SUPPORTED_PROTOCOLS))
        ...
        if _port is None and _proto in KNOWN_PROTOCOLS:
            ...
            try :
                ...
                raise usage.UsageError("invalid `port`, '%s', it must be int value." % _port)
    ...
    def opt_target(self, value):
        try :
            ...
            raise usage.UsageError("invalid format of `target`, '<host address or ip>:<port>'")
    ...
    def opt_timeout(self, value):
        try :
            ...
            raise usage.UsageError("invalid `timeout`, '%s', it must be int value." % value)
